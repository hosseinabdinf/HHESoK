...
func testHera128F(t *testing.T, index int) {
	//logger := HHESoK.NewLogger(HHESoK.DEBUG)
	tc := SymHera.TestVector[index+4]
	heraCipher := SymHera.NewHera(tc.Key, tc.Params)
	encryptor := heraCipher.NewEncryptor()

	numRound := tc.Params.GetRounds()
	heParamIndex := index
	radix := 0
	fullCOEFF := false
	dataSize := len(tc.Plaintext)
	...
	hbtpParams := ckks_fv.RtFHeraParams[heParamIndex]
	...
	if numRound == 4 {
		heraModDown = ckks_fv.HeraModDownParams80[heParamIndex].CipherModDown
		stcModDown = ckks_fv.HeraModDownParams80[heParamIndex].StCModDown
	} else {
		heraModDown = ckks_fv.HeraModDownParams128[heParamIndex].CipherModDown
		stcModDown = ckks_fv.HeraModDownParams128[heParamIndex].StCModDown
	}
	...
	if fullCOEFF {
		...
		//}
		keystream = encryptor.KeyStream(dataSize)
